 ...
 * }
 */
Ext.lion.ScriptLoader( 'bp/scripts/component/FlowProcessToolBar.js' );
FlowWorkbenchPanel = function(id,title,config){
	...
	FlowWorkbenchPanel.superclass.constructor.call(this);
}

Ext.extend(FlowWorkbenchPanel, Ext.Panel, {
	op : 'v',
	closable : true,
	autoScroll : true,
	border : false,
	...
	initComponent : function(){
		FlowWorkbenchPanel.superclass.initComponent.call(this);
...
//	},
	...
	loadBpi : function(bpiId){
		...
		conn.open("GET", 'workbench/forProcessBpi.html?id='+bpiId, false);
		conn.send(null);
        ...
        if (responseStr.root.length > 1) {//多于一个任务实例
        	...
        	scope.setTitle("流程");
//        	if(! responseStr.root[0].jbpmTaskInstanceID){ scope.doLayout();}
        	scope.load({
