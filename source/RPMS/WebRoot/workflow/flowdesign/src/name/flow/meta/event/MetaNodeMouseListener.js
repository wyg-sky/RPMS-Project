    ...
    switch (state) {
      case StateMonitor.SELECT:
        ...
        if ((selectedTransitionModels.size() <= 1) && (selectedTransitionModels.size() <= 1) && (!e.ctrlKey)) {
            ...
            if('play'==operType){
            	if(selectedMetaNodeModel!=this.metaNodeModel){
	            	selectedMetaNodeModel = this.metaNodeModel;
	            	loadNodeExecuteInfoGrid(this.metaNodeModel.getText());
            	}
            }else
            	propertyPanel.modelClick(this.metaNodeModel);
            //lingzj 当选中改节点时，东部区域的属性页面，显示该节点属性
            //alert(this.metaNodeModel.id);
            /*if(workFlowPageType=='view'){
            	if(selectedMetaNodeModel!=this.metaNodeModel){
	            	selectedMetaNodeModel = this.metaNodeModel;
	            	window.parent.loadNodeExecuteInfoGrid(this.metaNodeModel.getText());
            	}
            }else
            	window.parent.showNodeTab(this.metaNodeModel.id);*/
            //:~
