...
WrapperMetaMouseListener.prototype.onClick = function (e) {
    ...
    var flowModel = this.wrapper.getModel();
    ...
    if (metaNodeModel !== null) {
        if (flowModel.getMetaNodeModels().size() >= FlowWorkSpace.META_NODE_MAX) {
        	//节点数不能超过:
            alert("\u8282\u70b9\u6570\u4e0d\u80fd\u8d85\u8fc7:" + FlowWorkSpace.META_NODE_MAX);
            return;
        }
        metaNodeModel.setPosition(point);
        flowModel.addMetaNodeModel(metaNodeModel);
