 ...
 * @author itsm
 */
function FlowTableViewer(model, ui) {
    this.base = ScrollPanel;
    this.base(ui);

    //
    this.setClassName("NAME_ITSM_UI_FONT NAME_ITSMFLOW_TALBE_VIEWER");

    //
    this.table = Toolkit.newTable();
    this.table.className = "NAME_ITSMFLOW_TABLE";
    this.add(this.table);
...
FlowTableViewer.prototype = new ScrollPanel();
FlowTableViewer.prototype.setModel = function (model) {
    if (!model) {
        return;
    }
    if (this.model == model) {
...
FlowTableViewer.prototype.getModel = function (model) {
    return this.model;
};
FlowTableViewer.prototype.update = function (observable, arg) {
...
//
FlowTableViewer.prototype.init = function () {
	//clear
    this.clearTable();
...
FlowTableViewer.prototype.addMetaNodeModel = function (metaNodeModel) {
    if (!metaNodeModel) {
        return;
    }
    var row = this.addRow();
...
FlowTableViewer.prototype.addRow = function () {
    var row = this.table.insertRow();
    this.rows.add(row);
    return row;
};
FlowTableViewer.prototype.clearTable = function () {
    Toolkit.clearElement(this.table);
    this.rows.clear();
};
FlowTableViewer.prototype.removeRow = function (row) {
    if (!row) {
        return;
    }
