SingleSystemAuthorizeWindow = function(config){
	Ext.apply(this,config);
	
	var html = '<table align="center"><tr height="22">'
			+ '<td align="center"><font color="red" size="2">未授权的单点系统</font></td><td></td><td align="center"><font color="red" size="2">已授权的单点系统</font></td></tr><tr>'
	...
	SingleSystemAuthorizeWindow.superclass.constructor.call(this, {
		title : '['+this.roleName+']的单点系统权限设置',
		layout : 'fit',
		html : html,
		...
		buttons : [{
			...
			iconCls : 'reset-icon',
			handler : function(){
				this.loadData({roleId : this.roleId});
			}
		},{
			text : '关闭',
			width : 55,
...
Ext.extend(SingleSystemAuthorizeWindow, Ext.Window,{
	
	loadData : function(params){
		Ext.Ajax.request({
			url : 'system/listSingleSystemsForAuthorize.html',
			method : 'POST',
			params : params,
			scope : this,
	...
	submitForm : function(){
		...
		Ext.Ajax.request({
			url : 'system/saveSingleSystemsForRole.html',
			method : 'POST',
			params : {singleSystemIds : ids, roleId : this.roleId},
			scope : this,
			success : function(response, options) {
