...
Ext.lion.system.DocumentManager = function(config) {
	...
	this.listTypeTree.items.items[1].on('click',this.onTypeTreeClick, this);
...
Ext.extend(Ext.lion.system.DocumentManager, Ext.lion.LionBusinessManager, {
	...
	addDocType : function(){
		this.actionMode = '1';
		this.editable = true;
		this.showWin({title : '新增信息'},'system/document/edit');
	...
	editDocType : function(){
		...
		this.actionMode = '2';
		this.editable = true;
		this.showWin({title : '编辑信息'},'system/document/edit');
	...
	viewDocType : function(){
		...
		this.actionMode = '3';
		this.editable = false;
		this.showWin({title : '查看信息'},'system/document/edit');
		this.editForm.loadFormData({
			id : this.docTypeId
		});
	},
	...
	showMoveDocWin : function(){
		...
		if(records && records.length > 0){
			this.showWin({title : '文档迁移'},'system/document/movedoc');
		}
	},
